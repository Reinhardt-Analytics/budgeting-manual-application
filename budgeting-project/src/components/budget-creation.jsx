...
function Budget() {
  // Initialize state with sessionStorage persistence
  ...
  const [guideChartView, setGuideChartView] = useState("guide"); // 'guide' or 'chart' for the guide section
  const [hiddenCategories, setHiddenCategories] = useState(["Housing"]);
  const [categoryCount, setCategoryCount] = useState(16);
  const svgRef = useRef();

  // Save to sessionStorage whenever budgetData changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (guideChartView === "chart") {
      ...
      const chartAxis = getComputedColor("--chart-axis") || "#808080";
      const chartText = getComputedColor("--chart-text") || "#e5e5e5";
      ...
      const maxPercentage = Math.max(
        ...normalizedData.map((d) => d.normalizedValue)
      );
  ...
  const getTotalBudget = () => {
    return Object.entries(budgetData.budgets).reduce(
  ...
  const handleBudgetChange = (e) => {
    ...
    if (
      rawValue === "" ||
      (!isNaN(numValue) && validateBudgetInput(numValue, selectedCategory))
    ) {
  ...
  const handleIncomeChange = (e) => {
    ...
    if (
      rawValue === "" ||
      (!isNaN(numValue) && numValue >= 0 && numValue <= 10000000)
    ) {
      setBudgetData((prev) => ({
        ...prev,
  ...
  const calculateRemaining = () => {
    const totalBudget = Object.values(budgetData.budgets).reduce(
      (sum, amount) => sum + (parseFloat(amount) || 0),
      0
    );
      ...
      <div className="budget-page-container">
        <div className="budget-content-centered">
          ...
          <div className="budget-card">
            <div className="card-header">
              ...
              <p className="card-subtitle">
                Select a category and assign a value to continue.
              </p>
            </div>
                  ...
                  return (
                    <div key={category} className="key-item-static">
                      ...
                      <span className="key-percentage-static">
                        {amount > 0
                          ? getCategoryPercentage(amount).toFixed(1)
                          : "0.0"}
                        %
                      </span>
                    </div>
          ...
          <div className="budget-user-guide">
            <h3 className="guide-section-heading">
            ...
            <div className="view-toggle">
              <button
                className={guideChartView === "guide" ? "active" : ""}
                onClick={() => setGuideChartView("guide")}
              >
                Guide View
              </button>
              <button
                className={guideChartView === "chart" ? "active" : ""}
                onClick={() => setGuideChartView("chart")}
              >
                Chart View
              </button>
            </div>

            {guideChartView === "guide" && (
            ...
            {/* Chart Section */}
            {guideChartView === "chart" && (
              <div className="guide-chart-section">
                <h3 className="chart-title">Budget Distribution</h3>
                <p className="chart-tooltip">
                  Each axis represents a category, and the shaded area shows the
                  relative proportion of your total budget.
                </p>
                ...
                <p className="chart-bottom-tooltip">
                  The chart adjusts its scale based on your highest budget
                  percentage. Concentric circles represent percentage intervals,
                  helping you visualize how your budget is distributed across
                  all categories.
                </p>
              </div>
            )}
      ...
      {showLimitWarning && (
        ...
        >
          ...
          >
            ...
            <p>
              You have reached the maximum limit of 16 categories. Please delete
              a category before creating a new one.
            </p>
            <button
              onClick={() => setShowLimitWarning(false)}
              ...
              <div className="summary-item large-item underlined">
                ...
                <span className="summary-amount">
                  {budgetData.currencyMode === "dollars"
                    ? formatCurrency(
                        Object.values(budgetData.budgets).reduce(
                          (sum, amount) => sum + (parseFloat(amount) || 0),
                          0
                        )
                      )
